...
code <- "/Volumes/GoogleDrive/My\ Drive/Paper_VirHostome_CoV2/06_Code"
...
library(pastecs)
library(tidyverse)
source(file.path(code, "subLocation.r"))
...
allPPIs <- list(
    HPA = hpa_subcell,
    HuSCI = unique(merge(binary, hpa_subcell, by.x = "Ensembl.gene.ID", by.y = "Ensembl", all.x = TRUE)[, c(1, 4, 18)]),
